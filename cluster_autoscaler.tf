locals {
  ...
  cluster_autoscaler_iam_policy_document        = local.cluster_autoscaler_iam_policy_enabled ? one(data.aws_iam_policy_document.cluster_autoscaler[*].json) : try(local.argocd_helm_apps_set["cluster-autoscaler"]["iam_policy_document"], "{}")
  cluster_autoscaler_use_sts_regional_endpoints = local.cluster_autoscaler_enabled ? local.argocd_helm_apps_set["cluster-autoscaler"]["use_sts_regional_endpoints"] : false
}
